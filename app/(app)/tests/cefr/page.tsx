...
const SKILLS: Skill[] = [
    {
        ...
        testType: 'GRAMMAR',
        title: 'Grammar',
        description: 'Test your understanding of tenses, sentence structure, and usage across all CEFR levels.',
        icon: Pencil,
        href: '/tests/cefr/grammar',
        duration: '20 min',
    ...
    },
    {
        ...
        testType: 'VOCABULARY',
        title: 'Vocabulary',
        description: 'Assess your word knowledge with context-based questions from A1 to C2 word lists.',
        icon: BookOpen,
        href: '/tests/cefr/vocabulary',
        duration: '20 min',
        questions: '30 questions',
        color: '#0ea5e9',
    ...
    },
    {
        ...
        testType: 'READING',
        title: 'Reading',
        description: 'Short texts with comprehension questions that adapt to your level in real time.',
        icon: AlignLeft,
        href: '/tests/cefr/reading',
        duration: '20 min',
...
export default function CEFRPage() {
    ...
    const grammarQuota = quota?.quotas['GRAMMAR']
    ...
    return (
        <div className="w-full min-h-full bg-[#f8f9fd] px-5 md:px-8 py-6 flex flex-col gap-6">
            ...
            <PageBanner
                visible={visible}
                tag="CEFR Level Test"
                title={<>Find your <span style={{ color: '#c7d2fe' }}>English level.</span></>}
                subtitle="Adaptive questions from A1 to C2 — know exactly where you stand in under an hour."
                ctaLabel="Start level test"
                onCta={() => router.push('/tests/cefr/full')}
                stats={[
                    { icon: Clock, value: '45–60 min', label: 'Test duration' },
                    { icon: BarChart2, value: 'A1 – C2', label: 'Level range' },
                    { icon: CheckCircle, value: '3 sections', label: 'Grammar · Vocab · Reading' },
                ]}
            />
                    ...
                    {quotaLoading ? (
                        ...
                        <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-gray-100 shadow-sm">
                            <span className="text-sm font-bold" style={{ color: C1 }}>
                                {grammarQuota.limit === -1 ? '∞' : grammarQuota.remaining}
                            </span>
                            <span className="text-xs text-gray-400">Grammar left</span>
                        </div>
                    ) : (
            ...
            >
                Grammar is free &nbsp;•&nbsp; More sections coming soon &nbsp;•&nbsp; Results shown instantly
            </p>
        </div>
    )
}