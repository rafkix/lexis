...
function TestCard({
    ...
    return (
        <motion.div variants={cardItem} whileHover={{ y: -2 }} whileTap={{ scale: 0.985 }}>
            <Link
                href={href}
                className={`
...
export default function ReadingPage() {
    ...
    const latestBand = progress?.latest_band_score ?? 0
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                Promise.all([getMyHistory({ limit: 100 }), getMyProgress()])
                    .then(([hist, prog]) => { setHistory(hist); setProgress(prog) })
                    .catch(() => { })
            } finally {
                setLoading(false)
            }
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50">
            ...
            <header className="sticky top-0 z-30 bg-white border-b border-gray-200 h-[56px]">
                <div className={`${MAX_W} mx-auto px-6 h-full flex items-center`}>
                    ...
                    {/* Title — absolutely centred, never pushes back btn */}
