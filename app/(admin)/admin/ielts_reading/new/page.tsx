...
/** Question types that require an options array (A/B/C choices or heading bank). */
...
export default function AdminIeltsNewPage() {
    ...
    const handleSubmit = async () => {
        ...
        // Derive question_types automatically from all groups
        const questionTypes = collectQuestionTypes(parts)

        const payload: ReadingTestCreate = {
            global_title: title.trim(),
            is_active: true,
            is_free: isFree,
            question_types: questionTypes,
            parts: parts.map((p, i) => ({ ...p, part: i + 1 })),
        }

        setSaving(true)
        try {
            const created = await adminCreateTest(payload)
            setSuccess(true)
